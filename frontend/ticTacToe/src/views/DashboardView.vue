...
export default {
  ...
  methods: {
    ...
    joinGame(game) {
      if (game.status === "in_progress" || (game.player1 === this.user.id || game.player2 === this.user.id)) {
  ...
  async mounted() {
    ...
    for (const game of this.games) {
      if (!this.gamePlayersNames[game.id] || !this.gamePlayersNames[game.id].player1Name || !this.gamePlayersNames[game.id].player2Name) {
        ...
        await this.fetchPlayerNames(game, this.gamePlayersNames);
      }
    }
  }
}
          ...
          <li v-for="game in games" :key="game.id">
            <div class="bg-gray-100 p-2 rounded-lg">
              <div class="flex justify-between">
                ...
                <button v-if="(game.status === 'waiting') || (game.status === 'in_progress' && (game.player1 === user.id || game.player2 === user.id))" @click="joinGame(game)"
